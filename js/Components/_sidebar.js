(function () {
  ...
  document.querySelectorAll(".group_toggle_btn").forEach(function (btn) {
    btn.addEventListener("click", function () {
      ...
      const groupList = btn
        .closest(".history_group")
        .querySelector(".group_list");
      if (groupList) {
  ...
  document.addEventListener("click", function (e) {
    if (
      e.target.closest(".chat_hover_actions") ||
      e.target.closest(".popover_wrapper")
    ) {
      return;
    }
  ...
  document.addEventListener("click", function (e) {
    ...
    if (chatId === "pinned-1") {
      ...
      const chat = chatHistory.find(function (c) {
        return c.id === chatId;
      });
      if (chat) {
        chat.pinned = false;
  ...
  document.addEventListener("click", function (e) {
    ...
    if (typeof copyToClipboard === "function") {
      copyToClipboard(
        window.location.origin + window.location.pathname + "#" + chatId,
      ).then(function () {
  ...
  document.addEventListener("click", function (e) {
    ...
    const index = chatHistory.findIndex(function (c) {
      return c.id === chatId;
    });
    if (index > -1) {
  ...
  document.addEventListener("click", function (e) {
    ...
    document
      .querySelectorAll(".organize_popover .popover_item")
      .forEach(function (item) {
        item.classList.remove("active");
      });
  ...
  function loadChat(chatId) {
    ...
    chat.messages.forEach(function (msg) {
      if (msg.role === "user" && typeof appendUserMessage === "function") {
        ...
        msg.role === "assistant" &&
        typeof appendAssistantMessage === "function"
      ) {
